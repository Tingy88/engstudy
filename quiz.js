...
function switchInlineMode(mode, word) {
  ...
  if (mode === 'choice') {
    ...
    fetch('/api/gemini', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ contents:[{parts:[{text:`You are an expert English teacher for Thai students. Create 1 fill-in-the-blank question testing the correct grammatical use of the word "${word}". Rules: 1. Question MUST be a sentence with "___" to fill. 2. Options: A is the CORRECT answer. B, C, D MUST be common mistakes (wrong form, wrong tense). 3. "explanation" MUST be in clear, natural Thai. Explain WHY A is correct and briefly why others are wrong. Reply ONLY raw JSON: {"question":"...","options":["A...","B...","C...","D..."],"answer":"A","explanation":"อธิบายเป็นภาษาไทยชัดเจน"}}`}]}] }) })
