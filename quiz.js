...
function switchInlineMode(mode, word) {
  ...
  if (mode === 'choice') {
    ...
    fetch('/api/gemini', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ contents:[{parts:[{text:`You are a strict IELTS/TOEFL tutor. CEFR Level: ${STATE.level}. Target word: "${word}". Create 1 fill-in-the-blank sentence for "${word}". STRICT RULES: 1. The correct answer (A) MUST be "${word}" used with correct grammar for this level. 2. Options B, C, D MUST be common grammatical mistakes of the EXACT WORD "${word}" (e.g., wrong verb tense, missing plural 's', wrong preposition). DO NOT use synonyms (e.g., if word is "affect", DO NOT use "effect" as a distractor). 3. The sentence complexity MUST match ${STATE.level} standard. 4. "explanation" MUST be in advanced Thai, explaining the grammar rule and why the wrong forms are incorrect. Reply ONLY raw JSON: {"question":"...___...","options":["A ${word}...","B wrong_form_1","C wrong_form_2","D wrong_form_3"],"answer":"A","explanation":"อธิบายระดับ ${STATE.level} เป็นภาษาไทย"}}`}]}] }) })
...
function checkGrammarInline(word) {
  ...
  fetch('/api/gemini', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ contents:[{parts:[{text:`You are a strict IELTS writing examiner. CEFR Level: ${STATE.level}. The student MUST use the word "${word}". Evaluate STRICTLY based on ${STATE.level} standards. If the sentence is too simple (e.g., A1 level) but the target is ${STATE.level}, it is WRONG. Check grammar, spelling, collocations, and sentence complexity. Reply ONLY JSON: {"correct":true/false,"corrected":"a proper ${STATE.level} level sentence","errors":["specific error"],"explanation":"อธิบายระดับ ${STATE.level} เป็นภาษาไทย ถ้าผิดบอกว่าผิดตรงไหน ถูกแต่ยังไม่ถึงระดับก็บอกว่าควรยกเลเวลให้สูงขึ้น"}\nStudent wrote: \"${sentence}\"`}]}] }) })
  .then(r=>r.json()).then(data=>{
